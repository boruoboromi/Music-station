...
module.exports = function(app) {
	...
	app.post('/upload_work', function(req, res) {
		...
		form.parse(req, function(err, fields, files) { //解析
			...
			//保存到上传目录
			var filePath = saveFile(files.file, form)
			var imgPath = saveFileImg(files.fileImg, form);

			models.saveUploadToDB(fields, filePath, imgPath) //数据入库
			res.redirect('/index')
		})
	});
	...
	app.post("/musicZan", function(req, res) {
		var id = parseInt(req.body.id)
		models.checkMusicZan(id, "peyton", function(result) {
			if(result) {
				//没点过此作品的赞
				models.musicZanDo(id,"peyton")
				res.json({
					status: 0
				})
			} else {
	...
	//存mp3图片
	function saveFileImg(file, form) {
		var extName = ''; //后缀名
		switch(file.type) {
			case 'image/pjpeg':
				extName = '.jpg';
